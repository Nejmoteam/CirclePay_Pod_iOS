...
class MerchantEnablePaymentGatewayModule: XCTestCase {
    ...
    func test_Merchant_enable_payment_gateway() {
        //GIVEN
        stub(condition: enableMerchantPaymentGatewayStubCondition() ) { urlRequest in
        ...
        let expectation =  self.expectation(description: "Failed To Enable Merchant payment gateway")
        let paymentGatewayId = "610b2c486df621209c85215a"
        //WHEN
        CirclePay.merchants.enablePaymentGateway(paymentGatewayId: paymentGatewayId) { model, error in
    ...
    func enableMerchantPaymentGatewayStubCondition() -> HTTPStubsTestBlock {
        return { req in
            let urlCondition = req.url?.absoluteString.contains("merchants/payment/gateway/enable") ?? false
