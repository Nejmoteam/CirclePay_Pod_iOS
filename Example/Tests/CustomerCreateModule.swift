...
class CustomerCreateModule: XCTestCase {
    ...
    func test_Cutomer_Create() {
        //GIVEN
        stub(condition: createCustomerStubCondition() ) { urlRequest in
        ...
        var expectedResponse: CreateCustomerCodable? = nil
        let fName = "adel"
        let lName = "mohy"
        ...
        let city = "Cairo"
        let apt = "7"
        let expectation =  self.expectation(description: "Failed To Create Customer")
        //WHEN
        CirclePay.customers.createCustomer(firstName: fName, lastName: lName, address: address, country: country, governorate: governorate, city: city, aptNumber: apt, email: email, mobileNumber: mobile) { model, error in
    ...
    func createCustomerStubCondition() -> HTTPStubsTestBlock {
        return { req in
            let urlCondition = req.url?.absoluteString.contains("customer/create") ?? false
